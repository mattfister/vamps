...
class Seeker(Enemy):
    def __init__(self, x, y, facing, state):
        Enemy.__init__(self, x, y, facing, 3, [0, 0, 32, 32], state, 'sprites', [1*32, 9*32, 32, 32], state.batch, state.enemy_group)
    ...
    def create_animations(self):
        self.add_animation('walkRight', [[1*32, 9*32, 32, 32], [2*32, 9*32, 32, 32]], fps=10.0)
        self.add_animation('walkLeft', [[1*32, 10*32, 32, 32], [2*32, 10*32, 32, 32]], fps=10.0)
